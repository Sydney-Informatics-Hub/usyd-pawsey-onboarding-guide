...
Log in to the Pawsey `Origin` portal and follow the [instructions to create Acacia access keys](https://pawsey.atlassian.net/wiki/spaces/US/pages/51924486/Configuring+and+running+an+S3+client#ConfiguringandrunninganS3client-Authenticating). Note that separate keys are required per project storage (accessible to all members of the project) and per user storage (private to the user) on Acacia. 


:::{.callout-important}
Make sure you record your `Access ID` and `Secret Key`!
:::

**Step 2. Download S3 client application**

Log on to Setonix, and load the modules below, after first checking which are the latest versions of the tools. Note that Setonix does not allow modules to be loaded without specifying a version. 
...
module load python/<latest-version>
module load boto3/<latest-version>
```

Download the Pawsey client with git, ideally to your `/software` storage. The environment variable `$MYSOFTWARE` expands to `/software/projects/<pawsey-project-id>/<pawsey-user-id>`. 
...
Follow the [instructions to configure Rclone](https://pawsey.atlassian.net/wiki/spaces/US/pages/51924486/Configuring+and+running+an+S3+client#ConfiguringandrunninganS3client-Rclone) via `tee` command or direct text editing of your `$HOME/.config/rclone/rclone.conf` file to add your private `Access ID` and `Secret Key` generated at Step 1. 
...
- https://pawsey.atlassian.net/wiki/spaces/US/pages/51924486/Configuring+and+running+an+S3+client 


**Step 4. Add some test data to Acacia**


**Step 5. Mount your Acacia bucket locally**
